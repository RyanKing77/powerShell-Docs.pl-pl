...
**Add-PswaAuthorizationRule** polecenie cmdlet dodaje nową regułę autoryzacji do zestawu reguł autoryzacji dostępu do sieci Web PowerShell(r) Windows.
...
Na komputerze, który jest przyłączony do domeny usługi Active Directory polecenie cmdlet używa identyfikator zabezpieczeń (SID) komputera, aby utworzyć regułę. Dzięki temu można używać krótką nazwę, w pełni kwalifikowaną nazwę domeny (FQDN) lub adres IP dla **nazwy komputera** pola na stronie logowania.
...
Określa co najmniej jednego użytkownika, do których ta reguła udziela dostępu. Nazwa użytkownika może mieć konto użytkownika lokalnego na komputerze bramy lub użytkownika w usługach AD DS. Format jest `domain\user` lub `computer\user`.
...
To polecenie cmdlet obsługuje typowe parametry:

-Verbose, -Debug, -ErrorAction, -ErrorVariable, -OutBuffer i -OutVariable.
...
Ten przykład umożliwia wszystkim użytkownikom dostęp do wszystkich punktów końcowych na wszystkich komputerach. Wyłącza to zasadniczo reguły autoryzacji.

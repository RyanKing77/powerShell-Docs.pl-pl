...
Strumień nowych informacji strukturalnych można teraz używać do przesyłania danych strukturalnych między skrypt i jej hosta. **Write-Host** również została zaktualizowana w celu Emituj dane wyjściowe do strumienia informacji, których można teraz przechwytywania lub jego wyłączeniu. Nowy **informacji zapisu** polecenie cmdlet (wraz z odpowiedniego interfejsu API WriteInformation) używane z **- InformationVariable** i **- InformationAction** typowych parametrów zapewnia większą elastyczność i możliwość.

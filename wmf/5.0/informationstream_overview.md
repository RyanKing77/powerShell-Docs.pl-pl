...
Nowy strumień ze strukturą informacji teraz może służyć do przesyłania danych ze strukturą między skryptu i jej hosta. **Write-Host** został także zaktualizowany i emitowanie jej dane wyjściowe do strumienia informacje, którym można teraz przechwytywania lub wyciszyć go. Nowy **zapisuj informacji** polecenia cmdlet (wraz z odpowiedniego interfejsu API WriteInformation) za pomocą **- InformationVariable** i **- InformationAction** typowych parametrów zapewnia większą elastyczność i możliwości.

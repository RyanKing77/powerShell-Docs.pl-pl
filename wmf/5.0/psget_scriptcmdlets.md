...
ms.locfileid: "62057355"
---
# <a name="powershellget-cmdlets-for-script-management"></a><span data-ttu-id="ed298-102">Polecenia cmdlet PowerShellGet na potrzeby zarządzania skryptami</span><span class="sxs-lookup"><span data-stu-id="ed298-102">PowerShellGet Cmdlets for Script Management</span></span>

## <a name="find-script-cmdlet"></a><span data-ttu-id="ed298-103">Polecenia cmdlet Find-Script</span><span class="sxs-lookup"><span data-stu-id="ed298-103">Find-Script cmdlet</span></span>
<span data-ttu-id="ed298-104">Polecenia cmdlet Find-Script umożliwia odnajdywanie pliki skryptów z innych kryteriów wyszukiwania takich jak nazwa tagu, filtrowanie, nazwa polecenia, zakres wersji, dokładna wersja, wszystkie wersje, łącznie z jej zależnościami i za pomocą określonych lub wszystkich zarejestrowanych repozytoriów.</span><span class="sxs-lookup"><span data-stu-id="ed298-104">Find-Script cmdlet lets you to discover the script files with different search criteria like name, tag, filter, command name, version range, exact version, all versions, including its dependencies and from specific or all registered repositories.</span></span>

<span data-ttu-id="ed298-105">Przykład użycia:</span><span class="sxs-lookup"><span data-stu-id="ed298-105">Example usage:</span></span>
```powershell
...
## <a name="save-script-cmdlet"></a><span data-ttu-id="ed298-106">Polecenie cmdlet Save-Script</span><span class="sxs-lookup"><span data-stu-id="ed298-106">Save-Script cmdlet</span></span>
<span data-ttu-id="ed298-107">Polecenie cmdlet Save-Script umożliwia przeglądanie pliku skryptu przez zapisanie go do określonej lokalizacji.</span><span class="sxs-lookup"><span data-stu-id="ed298-107">Save-Script cmdlet lets you to review the script file by saving it to a specified location.</span></span>
```powershell
...
## <a name="install-script-and-get-installedscript-cmdlets"></a><span data-ttu-id="ed298-108">Polecenia cmdlet Install-Script i Get-InstalledScript</span><span class="sxs-lookup"><span data-stu-id="ed298-108">Install-Script and Get-InstalledScript cmdlets</span></span>
<span data-ttu-id="ed298-109">Polecenia cmdlet Install-Script pozwala zainstalować plik określony skrypt wraz z jego zależności w określonym zakresie.</span><span class="sxs-lookup"><span data-stu-id="ed298-109">Install-Script cmdlet lets you to install a specific script file along with its dependencies to the specified scope.</span></span> <span data-ttu-id="ed298-110">Domyślnie skrypty są instalowane w zakresie AllUsers.</span><span class="sxs-lookup"><span data-stu-id="ed298-110">By default, scripts are installed to the AllUsers scope.</span></span> <span data-ttu-id="ed298-111">Polecenie cmdlet Get-InstalledScript pozwala uzyskać listę plików skryptów, które zostały zainstalowane za pomocą polecenia cmdlet Install-Script.</span><span class="sxs-lookup"><span data-stu-id="ed298-111">Get-InstalledScript cmdlet lets you to get the list of script files which were installed using Install-Script cmdlet.</span></span>

<span data-ttu-id="ed298-112">Użyj Uwaga: Aby zezwolić na zarządzanie i lokalizowania skryptów, gdy są one zainstalowane, skrypt instalacji utworzysz domyślny folder do przechowywania skryptów w $home\Documents\WindowsPowerShell\Scripts i dodawanie folderu do środowiska ścieżki.</span><span class="sxs-lookup"><span data-stu-id="ed298-112">Use note: To allow management and locating of scripts once they are installed, Install-script will create a default folder for storing scripts at $home\Documents\WindowsPowerShell\Scripts, and add that folder to your PATH environment.</span></span> <span data-ttu-id="ed298-113">W przypadku modyfikowania ścieżki jest istotna, należy użyć Save-Script zamiast skryptu instalacji.</span><span class="sxs-lookup"><span data-stu-id="ed298-113">If modifying the path is a concern, use Save-Script instead of Install-Script.</span></span> <span data-ttu-id="ed298-114">Get-InstalledScripts i skrypt dezinstalacji może działać wyłącznie za pomocą skryptów umieszczone w systemie przy użyciu skryptu instalacji.</span><span class="sxs-lookup"><span data-stu-id="ed298-114">Get-InstalledScripts and Uninstall-Script can only work with scripts placed on the system using Install-Script.</span></span>
```powershell
...
<span data-ttu-id="ed298-115">Można również użyć polecenia Get — nazwa &lt;InstalledScriptFileName&gt; go.</span><span class="sxs-lookup"><span data-stu-id="ed298-115">You can also use Get-Command –Name &lt;InstalledScriptFileName&gt; to get it.</span></span> <span data-ttu-id="ed298-116">Zainstaluj dwie lokalizacje są dodawane do zmiennej środowiskowej PATH przy pierwszym użyciu określonego zakresu.</span><span class="sxs-lookup"><span data-stu-id="ed298-116">Two install locations are added to the PATH environment variable on first use of a specified scope.</span></span>
```powershell
...
## <a name="update-script-cmdlet"></a><span data-ttu-id="ed298-117">Polecenie cmdlet Update-Script</span><span class="sxs-lookup"><span data-stu-id="ed298-117">Update-Script cmdlet</span></span>
<span data-ttu-id="ed298-118">Polecenie cmdlet Update-Script umożliwia w miejscu aktualizacji pliki skryptów, które zostały zainstalowane za pomocą polecenia cmdlet Install-Script.</span><span class="sxs-lookup"><span data-stu-id="ed298-118">Update-Script cmdlet lets you to do in-place update of the script files which were installed using Install-Script cmdlet.</span></span>
```powershell
...
## <a name="uninstall-script-cmdlet"></a><span data-ttu-id="ed298-119">Polecenia cmdlet odinstalować skryptu</span><span class="sxs-lookup"><span data-stu-id="ed298-119">Uninstall-Script cmdlet</span></span>
```powershell
...
## <a name="new-scriptfileinfo-and-test-scriptfileinfo-cmdlets"></a><span data-ttu-id="ed298-120">Polecenia cmdlet nowe ScriptFileInfo i ScriptFileInfo testu</span><span class="sxs-lookup"><span data-stu-id="ed298-120">New-ScriptFileInfo and Test-ScriptFileInfo cmdlets</span></span>
<span data-ttu-id="ed298-121">ScriptFileInfo nowe polecenie cmdlet pozwala utworzyć nowy plik skryptu z metadanymi, takich jak wersja, Guid, autor i opis, itp. Polecenia cmdlet test-ScriptFileInfo umożliwia weryfikowanie i Pobierz metadane pliku skryptu.</span><span class="sxs-lookup"><span data-stu-id="ed298-121">New-ScriptFileInfo cmdlet lets you to create a new script file with metadata like Version, Guid, Author, and Description, etc. Test-ScriptFileInfo cmdlet lets you to validate and get the script file metadata.</span></span>
```powershell
...
## <a name="update-scriptfileinfo-cmdlet"></a><span data-ttu-id="ed298-122">Update-ScriptFileInfo cmdlet</span><span class="sxs-lookup"><span data-stu-id="ed298-122">Update-ScriptFileInfo cmdlet</span></span>
<span data-ttu-id="ed298-123">Polecenie cmdlet Update-ScriptFileInfo pozwala zaktualizować istniejące metadanych pliku skryptu.</span><span class="sxs-lookup"><span data-stu-id="ed298-123">Update-ScriptFileInfo cmdlet lets you to update the existing script file metadata.</span></span>
```powershell
...
## <a name="register-psrepository-and-set-psrepository-cmdlets-with-script-sharing-support"></a><span data-ttu-id="ed298-124">Polecenia cmdlet Register-PSRepository i Set-PSRepository w ze skryptem udostępnianie pomocy technicznej</span><span class="sxs-lookup"><span data-stu-id="ed298-124">Register-PSRepository and Set-PSRepository cmdlets with script sharing support</span></span>
<span data-ttu-id="ed298-125">Użyj polecenia cmdlet Register-PSRepository/Set-PSRepository, aby dodać **ScriptSourceLocation** i **ScriptPublishLocation** do PSRepository.</span><span class="sxs-lookup"><span data-stu-id="ed298-125">Use Register-PSRepository/Set-PSRepository cmdlets to add the **ScriptSourceLocation** and **ScriptPublishLocation** to the PSRepository.</span></span>
```powershell
...
## <a name="publish-script-cmdlet"></a><span data-ttu-id="ed298-126">Polecenia cmdlet skryptów publikowania</span><span class="sxs-lookup"><span data-stu-id="ed298-126">Publish-Script cmdlet</span></span>
<span data-ttu-id="ed298-127">Polecenia cmdlet skryptów publikowania umożliwia publikowanie pliku skryptu z metadanymi prawidłowy, takie jak wersja, Guid, autor i opis, itp.</span><span class="sxs-lookup"><span data-stu-id="ed298-127">Publish-Script cmdlet lets you to publish your script file with valid metadata like Version, Guid, Author, and Description, etc.</span></span>
```powershell

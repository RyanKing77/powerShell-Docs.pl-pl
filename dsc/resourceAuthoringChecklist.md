...
Sprawdź schematu zasobów (*. schema.mof) pliku. Możesz użyć [projektancie zasobów DSC](https://www.powershellgallery.com/packages/xDSCResourceDesigner) ułatwiające tworzenie i testowanie schematu.
Upewnij się, że:
...
Ponadto należy używać **xDscResource testu** i **xDscSchema testu** polecenia cmdlet z [projektancie zasobów DSC](https://www.powershellgallery.com/packages/xDSCResourceDesigner) umożliwia automatyczne weryfikowanie zasobu i schematu:
...
Jedną z podstawowych cech zasoby DSC jest idempotentność. Oznacza to, że trwa operacja zastosowania konfiguracji DSC, zawierająca ten zasób wielokrotnie będzie zawsze ten sam efekt. Jeśli na przykład możemy utworzyć konfigurację, która zawiera następujące zasoby pliku:
...
- : Największy problem z komunikatami o błędach jest często nie istnieje, dlatego upewnij się, że są one dostępne.
- Łatwe do zrozumienia: kody błędów ludzkich możliwy do odczytu, nie zasłoniętej
- Dokładny: Opisano, jakie dokładnie problem
- Zwyczajowo: Doradztwo jak rozwiązać ten problem
...
## <a name="best-practice-resource-module-contains-tests-folder-with-resourcedesignertestsps1-script"></a>Najlepsze rozwiązanie: moduł zasobów zawiera folder testy za pomocą skryptu ResourceDesignerTests.ps1
...
## <a name="best-practice-resource-folder-contains-resource-designer-script-for-generating-schema"></a>Najlepsze rozwiązanie: folder zasobów zawiera zasób skryptu projektanta do generowania schematu
...
## <a name="best-practice-resource-supports--whatif"></a>Najlepsze rozwiązanie: zasób obsługuje - WhatIf

...
ms.locfileid: "58055703"
---
# <a name="resource-dependencies-using-dependson"></a><span data-ttu-id="52649-103">Zależności zasobów z użyciem właściwości DependsOn</span><span class="sxs-lookup"><span data-stu-id="52649-103">Resource dependencies using DependsOn</span></span>

<span data-ttu-id="52649-104">Podczas wpisywania [konfiguracje](configurations.md), możesz dodać [bloków zasobów](../resources/resources.md) skonfigurować aspekty elementu docelowego węzła.</span><span class="sxs-lookup"><span data-stu-id="52649-104">When you write [Configurations](configurations.md), you add [Resource blocks](../resources/resources.md) to configure aspects of a target Node.</span></span> <span data-ttu-id="52649-105">W miarę postępu dodawania zasobów bloki konfiguracje powiększać tak, bardzo duże i skomplikowane do zarządzania.</span><span class="sxs-lookup"><span data-stu-id="52649-105">As you continue to add Resource blocks, your Configurations can grow quite large and cumbersome to manage.</span></span> <span data-ttu-id="52649-106">Jednym z wyzwań jest kolejność stosowana z bloków zasobów.</span><span class="sxs-lookup"><span data-stu-id="52649-106">One such challenge is the applied order of your resource blocks.</span></span> <span data-ttu-id="52649-107">Zazwyczaj zasoby są stosowane w kolejności, w którym są zdefiniowane w ramach konfiguracji.</span><span class="sxs-lookup"><span data-stu-id="52649-107">Typically resources are applied in the order they are defined within the Configuration.</span></span> <span data-ttu-id="52649-108">Wraz z rozwojem większej i bardziej złożonej konfiguracji można użyć `DependsOn` klawisz, aby zmienić kolejność zastosowane zasobów, określając, na które zasobem, który jest zależny od innego zasobu.</span><span class="sxs-lookup"><span data-stu-id="52649-108">As your Configuration grows larger and more complex, you can use the `DependsOn` key to change the applied order of your resources by specifying that a resource depends on another resource.</span></span>

<span data-ttu-id="52649-109">`DependsOn` Klucz może zostać użyty w bloku zasobów.</span><span class="sxs-lookup"><span data-stu-id="52649-109">The `DependsOn` key can be used in any Resource block.</span></span> <span data-ttu-id="52649-110">Jest ona zdefiniowana z ten sam mechanizm klucz/wartość, jak inne klucze zasobu.</span><span class="sxs-lookup"><span data-stu-id="52649-110">It is defined with the same key/value mechanism as other Resource keys.</span></span> <span data-ttu-id="52649-111">`DependsOn` Klucz oczekuje, że tablica ciągów przy użyciu następującej składni.</span><span class="sxs-lookup"><span data-stu-id="52649-111">The `DependsOn` key expects an array of strings with the following syntax.</span></span>

```
DependsOn = '[<Resource Type>]<Resource Name>', '[<Resource Type>]<Resource Name'
```

<span data-ttu-id="52649-112">Poniższy przykład umożliwia skonfigurowanie reguły zapory po umożliwia włączenie i skonfigurowanie profilu publicznego.</span><span class="sxs-lookup"><span data-stu-id="52649-112">The following example configures a firewall rule after enabling and configuring the public profile.</span></span>
...
<span data-ttu-id="52649-113">Po zastosowaniu konfiguracji profilu zapory są zawsze skonfigurowane po raz pierwszy niezależnie od tego, w jakiej kolejności bloków zasobów są zdefiniowane.</span><span class="sxs-lookup"><span data-stu-id="52649-113">When you apply the Configuration, the firewall profile will always be configured first regardless of which order the Resource blocks are defined.</span></span> <span data-ttu-id="52649-114">Możesz zastosować konfigurację, należy Pamiętaj węzłów docelowych istniejącej konfiguracji, aby można było przywrócić w razie potrzeby.</span><span class="sxs-lookup"><span data-stu-id="52649-114">If you apply the Configuration, be sure to note your target Nodes existing Configuration so you can revert if desired.</span></span>
...
<span data-ttu-id="52649-115">To gwarantuje również, że jeśli **FirewallProfile** zasobu nie powiedzie się z jakiegokolwiek powodu **zapory** blok nie zostanie wykonany, nawet jeśli została zdefiniowana najpierw.</span><span class="sxs-lookup"><span data-stu-id="52649-115">This also ensures that if the **FirewallProfile** resource fails for any reason, the **Firewall** block will not execute even though it was defined first.</span></span> <span data-ttu-id="52649-116">`DependsOn` Klucz zapewnia większą elastyczność w grupowania bloków zasobów i sprawdzeniu, czy zależności zostały rozwiązane przed wykonaniem zasobu.</span><span class="sxs-lookup"><span data-stu-id="52649-116">The `DependsOn` key allows more flexibility in grouping resource blocks and ensuring that dependencies are resolved before a Resource executes.</span></span>

<span data-ttu-id="52649-117">W bardziej zaawansowanych ustawień konfiguracji, można również użyć [zależności między węzłem](crossNodeDependencies.md) umożliwia bardziej szczegółową kontrolę (na przykład zapewnienie kontroler domeny jest skonfigurowana przed dołączeniem klientów do domeny).</span><span class="sxs-lookup"><span data-stu-id="52649-117">In more advanced Configurations, you can also use [Cross Node Dependency](crossNodeDependencies.md) to allow even more granular control (For example, ensuring a domain controller is configured before joining a client to the domain).</span></span>

## <a name="cleaning-up"></a><span data-ttu-id="52649-118">Czyszczenie</span><span class="sxs-lookup"><span data-stu-id="52649-118">Cleaning Up</span></span>

<span data-ttu-id="52649-119">Po zastosowaniu powyższych konfiguracji, należy wycofać kluczy w celu cofnięcia wszystkich zmian.</span><span class="sxs-lookup"><span data-stu-id="52649-119">If you applied the Configuration above, you can reverse keys to undo any changes.</span></span> <span data-ttu-id="52649-120">W powyższym przykładzie ustawienie **włączone** spowoduje wyłączenie klucza na wartość false, reguły zapory i profilu.</span><span class="sxs-lookup"><span data-stu-id="52649-120">In the above example, setting the **Enabled** key to false will disable the firewall rule and profile.</span></span> <span data-ttu-id="52649-121">Przykładu należy zmodyfikować odpowiednio do potrzeb, aby dopasować poprzedni stan skonfigurowanego Twój węzeł docelowy.</span><span class="sxs-lookup"><span data-stu-id="52649-121">You should modify the example as needed to match your target Node's previous configured state.</span></span>
...
## <a name="see-also"></a><span data-ttu-id="52649-122">Zobacz też</span><span class="sxs-lookup"><span data-stu-id="52649-122">See also</span></span>

- [<span data-ttu-id="52649-123">Użyj zależności między węzła</span><span class="sxs-lookup"><span data-stu-id="52649-123">Use Cross Node Dependencies</span></span>](./crossNodeDependencies.md)

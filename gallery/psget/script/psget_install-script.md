...
Polecenia cmdlet Install-skryptu uzyskuje ładunku skryptu z repozytorium, sprawdza, czy ładunku jest nieprawidłowy skrypt programu PowerShell i kopiuje plik skryptu do lokalizacji określonej instalacji.

Repozytoria domyślne, które operacjach skrypt instalacji są konfigurowane za pomocą rejestru PSRepository, zestaw PSRepository, Unregister-PSRepository i polecenia cmdlet Get-PSRepository. Podczas pracy z wielu repozytoriów, skrypt instalacji instaluje pierwszy skrypt, który odpowiada określonym kryteriom wyszukiwania (nazwa, MinimumVersion lub MaximumVersion) z pierwszego repozytorium bez żadnych błędów.


Skrypt instalacji polecenie cmdlet pobiera jednego lub większej liczby modułów z galerii online, weryfikuje i instaluje je na komputerze lokalnym do zakresu określonej instalacji.

Skrypt instalacji polecenie cmdlet pobiera jednego lub większej liczby modułów spełniające określone kryteria z galerii online, sprawdza, czy wyniki wyszukiwania są prawidłowe moduły i kopiowania modułu folderów do lokalizacji instalacji.
...
- Brak obsługi Side-by-side wersji plików skryptów środowiska PowerShell
- Obsługa instalacji zależności skryptu
...
Symbole wieloznaczne nie są obsługiwane w — nazwa Odinstaluj moduł instalacji, Zapisz-Module-modułu, skrypt instalacji skryptu Zapisz i polecenia cmdlet Uninstall-skryptu.
...
PowerShellGet obsługuje obsługę wersji side-by-side (SxS) modułu w skrypcie instalacji skrypt aktualizacji i poleceń cmdlet Publish-skryptu uruchamiających w programie Windows PowerShell 5.0 lub nowszej.

### <a name="install-script-examples"></a>Przykłady skryptów instalacji
...
## <a name="installing-a-script-with-dependent-scripts-and-modules"></a>Instalowanie skryptu z zależnych skryptów i modułów

...
ms.locfileid: "62080663"
---
# <a name="implementing-custom-authorization-for-a-management-odata-web-service"></a><span data-ttu-id="30415-102">Implementowanie autoryzacji niestandardowej na potrzeby internetowej usługi OData zarządzania</span><span class="sxs-lookup"><span data-stu-id="30415-102">Implementing Custom Authorization for a Management OData web service</span></span>

<span data-ttu-id="30415-103">Korzystanie z usługi sieci Web programu Windows PowerShell wymaga innych firm w celu zaimplementowania [Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) interfejsu do udostępnienia poleceń cmdlet programu Windows PowerShell.</span><span class="sxs-lookup"><span data-stu-id="30415-103">Using the Windows PowerShell Web Service requires a third party to implement the [Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) interface to expose Windows PowerShell cmdlets.</span></span> <span data-ttu-id="30415-104">Ten interfejs wykonuje autoryzacji użytkowników do usługi sieci web.</span><span class="sxs-lookup"><span data-stu-id="30415-104">This interface performs user authorization to the web service.</span></span> <span data-ttu-id="30415-105">Po zapisaniu kod implementujący interfejs, należy skompilować je do biblioteki DLL, który ma być używany w aplikacji sieci web.</span><span class="sxs-lookup"><span data-stu-id="30415-105">After writing the code to implement the interface, you must compile it into a DLL to be used in the web application.</span></span>

## <a name="pass-through-authorization"></a><span data-ttu-id="30415-106">Przekazywane autoryzacji</span><span class="sxs-lookup"><span data-stu-id="30415-106">Pass-through authorization</span></span>

<span data-ttu-id="30415-107">Najprostszym sposobem wdrożenia [Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) interfejs jest przekazywane implementację, która zezwala wszystkim użytkownikom.</span><span class="sxs-lookup"><span data-stu-id="30415-107">The simplest way to implement the [Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) interface is a pass-through implementation that authorizes all users.</span></span> <span data-ttu-id="30415-108">W tym przykładzie zapewnia nie zabezpieczeń i s podajemy tylko jako ilustruje sposób implementacji interfejsu.</span><span class="sxs-lookup"><span data-stu-id="30415-108">This example provides no security, and s provided only as an illustration of how to implement the interface.</span></span> <span data-ttu-id="30415-109">Implementacja [Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) interfejsu przesłonięcie dwóch metod: [Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser) i [Microsoft.Management.Odata.CustomAuthorization.GetMembershipId](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.GetMembershipId).</span><span class="sxs-lookup"><span data-stu-id="30415-109">An implementation of the  [Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) interface must override two methods: [Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser) and [Microsoft.Management.Odata.CustomAuthorization.GetMembershipId](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.GetMembershipId).</span></span> <span data-ttu-id="30415-110">W tym przykładzie [Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser) zawsze zwraca **System.Security.Principal.WindowsIdentity** obiekt skojarzony z bieżącego użytkownika.</span><span class="sxs-lookup"><span data-stu-id="30415-110">In this example, the [Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser) always returns the **System.Security.Principal.WindowsIdentity** object associated with the current user.</span></span>
...
### <a name="role-based-authorization"></a><span data-ttu-id="30415-111">Autoryzacja oparta na rolach</span><span class="sxs-lookup"><span data-stu-id="30415-111">Role-based authorization</span></span>

<span data-ttu-id="30415-112">Poniższy przykład wykonuje zasady autoryzacji opartej na rolach.</span><span class="sxs-lookup"><span data-stu-id="30415-112">The following example implements a role-based authorization policy.</span></span> <span data-ttu-id="30415-113">Zasada jest zdefiniowana w pliku XML, który znajduje się w katalogu głównym aplikacji z pliku web.config i pliki schematów mapowania pliku MOF i XML.</span><span class="sxs-lookup"><span data-stu-id="30415-113">The policy is defined in an XML file that resides in the main application directory with the web.config and MOF and XML mapping schema files.</span></span> <span data-ttu-id="30415-114">Aby uzyskać informacje o sposobie konfigurowania pliku schematu autoryzacji, zobacz [autoryzacji opartej na rolach Konfigurowanie](./configuring-role-based-authorization.md).</span><span class="sxs-lookup"><span data-stu-id="30415-114">For information about how to configure the authorization schema file, see [Configuring Role-based Authorization](./configuring-role-based-authorization.md).</span></span> <span data-ttu-id="30415-115">Pierwsza część próbki implementuje [Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser) i [Microsoft.Management.Odata.CustomAuthorization.GetMembershipId](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.GetMembershipId) metody.</span><span class="sxs-lookup"><span data-stu-id="30415-115">The first part of the sample implements the [Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser) and [Microsoft.Management.Odata.CustomAuthorization.GetMembershipId](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.GetMembershipId) methods.</span></span> <span data-ttu-id="30415-116">W tym przypadku metody interfejsu mogą wywoływać metod `RbacSystem` klasy (zdefiniowany poniżej), które wykonują rzeczywistą pracę sprawdzania uprawnień dla użytkownika.</span><span class="sxs-lookup"><span data-stu-id="30415-116">In this case, the interface methods call methods in the `RbacSystem` class (defined below) that do the actual work of checking the permissions for the user.</span></span>
...
<span data-ttu-id="30415-117">Poniższy przykład tworzy klasę, która analizuje zasady autoryzacji w pliku XML.</span><span class="sxs-lookup"><span data-stu-id="30415-117">The following example creates a class that parses authorization policy in the XML file.</span></span>
...
```
 ...
 <span data-ttu-id="30415-118">Następujące klasy reprezentowania grup i użytkowników, odpowiednio.</span><span class="sxs-lookup"><span data-stu-id="30415-118">The following classes represent groups and users, respectively.</span></span>
...
<span data-ttu-id="30415-119">Na koniec klasa RbacSystem implementuje metody, które wykonają tę pracę sprawdzania uprawnień dla użytkownika i przywrócenie stanu autoryzacji metody zdefiniowane w implementacji [Microsoft.Management.Odata.CustomAuthorization ](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) interfejsu.</span><span class="sxs-lookup"><span data-stu-id="30415-119">Finally, the RbacSystem class implements methods that do the work of checking the permissions for the user and return the authorization status to the methods defined in the implementation of the [Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) interface.</span></span>